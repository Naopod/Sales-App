...
def _compute_fiscal_quarter(dt: pd.Series, fiscal_year_start_month: int = 4) -> pd.Series:
...
def _build_period_columns(
    ...
    if time_granularity == 'quarter':
        fq = _compute_fiscal_quarter(dt, fiscal_year_start_month=fiscal_year_start_month)
        out['_fq'] = fq
        ...
        if 'Fiscal_Quarter' in out.columns:
            out['_period_label'] = out['Fiscal_Quarter'].astype(str).where(out['Fiscal_Quarter'].notna(), out['_period_label'])
        return out

    # year fiscal
    out['_period_start'] = pd.to_datetime({'year': (fy - 1).astype(int), 'month': fiscal_year_start_month, 'day': 1}, errors='coerce')
    out['_period_label'] = 'FY' + fy.astype(str)
    if 'Fiscal_Year_Label' in out.columns:
        out['_period_label'] = out['Fiscal_Year_Label'].astype(str).where(out['Fiscal_Year_Label'].notna(), out['_period_label'])
    return out
...
def analyze_client_anomalies(
    ...
    date_col = _pick_col(df, ['Date Fact.', 'Date_Ref', 'Date', 'Date facture', 'Date Facture'])
    amount_col = _pick_col(df, ['Montant', 'CA', 'CA Net', "Chiffre d'Affaires"]) 
    qty_col = _pick_col(df, ['Quantité', 'Quantite', 'Qty', 'Qte', 'Qté', 'Quantity'])
    order_col = _pick_col(df, ['N° Bon', 'No Bon', 'N Bon', 'Num Bon', 'Numéro Bon', 'Commande', 'N° Commande', 'Order'])
    ...
    df2 = df[[c for c in [client_col, date_col, amount_col, qty_col, order_col, 'Fiscal_Quarter', 'Fiscal_Year_Label'] if c and c in df.columns]].copy()
    ...
    df2 = _build_period_columns(df2, df2[date_col], tg, fiscal_year_start_month=fiscal_year_start_month)
    ...
    grouped = grouped.sort_values(['Client', '_period_start'], kind='mergesort').reset_index(drop=True)
    ...
    for m in present_metrics:
        grouped[f'z_{m.lower()}'] = grouped.groupby('Client', dropna=False)[m].transform(
    ...
    grouped['_is_anomaly'] = grouped['_score'].notna() & (grouped['_score'] >= float(z_thresh))
    ...
    if pd.notna(latest_start):
        latest_label = grouped.loc[grouped['_period_start'].eq(latest_start), '_period_label'].dropna().astype(str).head(1).tolist()
        latest_label = latest_label[0] if latest_label else None

    latest = grouped[grouped['_period_start'].eq(latest_start)].copy() if pd.notna(latest_start) else grouped.iloc[0:0].copy()
    latest_anom = latest[latest['_is_anomaly']].sort_values('_score', ascending=False).head(int(top_n))

    all_anom = grouped[grouped['_is_anomaly']].sort_values('_score', ascending=False).head(int(top_n))
    ...
    for _, r in latest_anom.iterrows():
        ...
        reason = ', '.join([f"{m}: z={z:+.2f}" for m, z in parts[:3]]) if parts else None

        top_rows.append({
            'client': '' if pd.isna(r.get('Client')) else str(r.get('Client')),
            'period': None if pd.isna(r.get('_period_label')) else str(r.get('_period_label')),
            'score': _safe_float(r.get('_score')),
            'main_metric': None if pd.isna(r.get('_main_metric')) else str(r.get('_main_metric')),
            'direction': None if pd.isna(r.get('_direction')) else str(r.get('_direction')),
    ...
    base['kpis'] = {
        ...
        'n_clients_anomalous_latest': int(latest_anom['Client'].nunique()) if latest_anom.shape[0] else 0,
    ...
    if include_scatter:
        try:
            ...
            base['graphs']['scatter_latest'] = viz_anomalies.create_client_anomalies_scatter(latest, z_thresh=float(z_thresh))
        except Exception:
...
def analyze_client_anomalies_for_client(
    ...
    keep_cols = [c for c in [client_col, date_col, amount_col, qty_col, order_col, 'Fiscal_Quarter', 'Fiscal_Year_Label'] if c and c in df_client.columns]
    ...
    grouped['Price'] = grouped['CA'] / grouped['Qty'].where(grouped['Qty'] > 0)
    grouped = grouped.drop(columns=[c for c in ['_line_count'] if c in grouped.columns])
    grouped = grouped.sort_values(['_period_start'], kind='mergesort').reset_index(drop=True)
    ...
    for _, r in grouped.iterrows():
        row = {
            ...
            'freq': _safe_float(r.get('Freq')),
            'price': _safe_float(r.get('Price')),
            'z_ca': _safe_float(r.get('z_ca')),
            'z_qty': _safe_float(r.get('z_qty')),
            'z_freq': _safe_float(r.get('z_freq')),
            'z_price': _safe_float(r.get('z_price')),
    ...
    for m in present_metrics:
        ...
        subset = grouped[grouped[flagcol] & grouped[zcol].notna()].copy()
        subset['_absz'] = subset[zcol].abs()
        subset = subset.sort_values('_absz', ascending=False).head(int(max_anomalies_per_metric))
        rows = []
        for _, r in subset.iterrows():
            rows.append({
                'period': None if pd.isna(r.get('_period_label')) else str(r.get('_period_label')),
                'value': _safe_float(r.get(m)),
                'z': _safe_float(r.get(zcol)),
    ...
    out['kpis'] = {
        ...
        'n_anomalies_ca': int(grouped.get('is_anomaly_ca', pd.Series(False)).sum()) if 'is_anomaly_ca' in grouped.columns else 0,
        'n_anomalies_qty': int(grouped.get('is_anomaly_qty', pd.Series(False)).sum()) if 'is_anomaly_qty' in grouped.columns else 0,
        'n_anomalies_freq': int(grouped.get('is_anomaly_freq', pd.Series(False)).sum()) if 'is_anomaly_freq' in grouped.columns else 0,
        'n_anomalies_price': int(grouped.get('is_anomaly_price', pd.Series(False)).sum()) if 'is_anomaly_price' in grouped.columns else 0,
    }
