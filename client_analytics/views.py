...
def _build_products_context(df: "pd.DataFrame", granularity: str = "month",
                             selected_product: str = "",
                             selected_family: str = "") -> dict:
    result = _safe_run(
...
def _build_clients_context(df: "pd.DataFrame", granularity: str = "month",
                            selected_client: str = "") -> dict:
    client_analysis = _safe_run(
        analysis_clients.analyze_clients, df,
        time_granularity=granularity, label="clients:analyze", fallback={}
    )
    client_options = _safe_run(
        analysis_clients.get_client_options, df, max_clients=500,
        label="clients:options", fallback=[]
    )
    ...
    if selected_client:
        def _port(g):
            return _safe_run(
                ...
                df, selected_client,
                time_granularity=g, top_n_families=8, top_n_products=10,
...
def _build_geographic_context(df: "pd.DataFrame", granularity: str = "month") -> dict:
    raw = _safe_run(
        analysis_geographic.analyze_geographic_data, df, granularity=granularity,
        label=f"geographic:{granularity}",
        fallback={
            "stats_zone": pd.DataFrame(), "stats_pays": pd.DataFrame(),
            "classif_summary": pd.DataFrame(), "top10_score": pd.DataFrame(),
            "geo_graphs": {},
        },
    )
...
def _build_currency_context(df: "pd.DataFrame", granularity: str = "month") -> dict:
    report = _safe_run(
        ...
        df, output_dir=None, granularity=granularity,
        label=f"currency:{granularity}", fallback={},
    )
    return {
...
def stats(request, pk):
    ...
    def _stat(g):
        return _safe_run(
            ...
            df_processed, granularity=g, skip_preprocessing=True,
    ...
    available_periods = _safe_run(
        data_processing.get_available_periods, df_processed,
        label="available_periods", fallback={}
    )
    temporal_metrics = _safe_run(
        analysis_time_series.get_temporal_metrics, df_processed,
        granularity=granularity, label="temporal_metrics", fallback={}
    )
    ...
    lead_time_analysis = _safe_run(
        aggregations.lead_time_pack, df_processed, period=period_param,
        label="lead_time", fallback={"error": "N/A"}
    )
    ...
    _empty_geo = {"stats_zone": [], "stats_pays": [], "classif_summary": [],
    ...
    context = {
        ...
        # Date info
        "date_min": (df_processed["Date Fact."].min()
                     if "Date Fact." in df_processed.columns else None),
        "date_max": (df_processed["Date Fact."].max()
                     if "Date Fact." in df_processed.columns else None),
        ...
        "client_analysis":    {}, "client_options":     [],
        "selected_client":    selected_client, "client_graphs": {},
        "top10_clients": None, "top20_clients": None,
        "concentration_top20_pct": None, "concentration_top10_pct": None,
        "client_portfolio": None, "client_portfolio_month": None,
        "client_portfolio_quarter": None, "client_portfolio_fiscal": None,
        "family_analysis":    {}, "family_analysis_month": {},
        "family_analysis_quarter": {}, "family_analysis_year": {},
        "family_analysis_fiscal": {}, "stats_famille": None, "family_graphs": {},
        "geo_analysis":       _empty_geo,
        "geo_analysis_month":   _empty_geo,
        "geo_analysis_quarter": _empty_geo,
        "geo_analysis_fiscal":  _empty_geo,
        "stats_pays": [], "stats_zone": [], "geo_graphs": {},
        "currency_kpi": {}, "currency_charts": {},
        "currency_kpi_month": {}, "currency_charts_month": {},
        "currency_kpi_quarter": {}, "currency_charts_quarter": {},
        "currency_kpi_fiscal": {}, "currency_charts_fiscal": {},
    }
